...
def summary_markdown(dataset: str, result: Mapping[str,Any]) -> str:
    ...
    run_scope = result.get("run_scope") or ("full_run_n1000" if int(result.get("n", 0) or 0) == 1000 else "smoke_test")
    header=["# ACE-RAG Evaluation Summary","",f"- dataset: {ds}",f"- ablation_variant: {result.get('ablation_variant','')}",f"- prompt_profile: {prompt}",f"- rendering_profile: {result.get('rendering_profile','structured_chain')}",f"- retrieval_variant: {result.get('retrieval_variant','full_hetero')}",f"- seed_selection_variant: {result.get('seed_selection_variant','global_seed_search')}",f"- residual_selection_variant: {result.get('residual_selection_variant','residual_lexical')}",f"- n: {result.get('n',0)}",f"- run_scope: {run_scope}",f"- generation_provider: {result.get('generation_provider','UNKNOWN')}",f"- created_at: {result.get('created_at','UNKNOWN')}",""]
