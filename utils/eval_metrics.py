...
def evaluate_predictions(rows: Sequence[Mapping[str,Any]], dataset: str | None=None, prompt_profile: str | None=None) -> dict[str,Any]:
    ...
    prompt_experiment_type=first_present(rows,"prompt_experiment_type", "main_comparison" if resolved_prompt=="common_qa" else "format_ablation" if resolved_prompt=="strict_short_qa" else "compact_prompt_ablation" if resolved_prompt in {"qmrag_compact_chain_qa","qmrag_compact_chain_light","qmrag_compact_chain_short_qa"} else "ablation" if resolved_prompt in {"qmrag_bundle_qa","qmrag_bundle_light","qmrag_bundle_tiny","qmrag_bundle_short_qa"} else "unknown")
