...
def summary_markdown(dataset: str, result: Mapping[str,Any]) -> str:
    ...
    header=["# QMRAG Evaluation Summary","",f"- dataset: {ds}",f"- prompt_profile: {prompt}",f"- rendering_profile: {result.get('rendering_profile','structured_chain')}",f"- n: {result.get('n',0)}",f"- generation_provider: {result.get('generation_provider','UNKNOWN')}",f"- created_at: {result.get('created_at','UNKNOWN')}",""]
