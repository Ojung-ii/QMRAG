...
def first_row(path: Path) -> Mapping[str, Any]:
...
def find_latest_full_context(output_root: Path, dataset: str, prompt_profile: str) -> Path:
    ...
    if not candidates:
        raise SystemExit(f"No full-context run found for dataset={dataset!r} prompt_profile={prompt_profile!r}")
...
def compare(left_path: Path, right_path: Path, analysis_dir: Path, dataset: str, prompt_profile: str) -> dict[str, Any]:
        ...
        pairs.append(
            {
                ...
                "right_top_bundles": right.get("top_bundles"),
                "right_context_token_budget": right.get("context_token_budget"),
                "right_ordering_source": right.get("ordering_source"),
                "right_rendered_bundle_count": right.get("rendered_bundle_count"),
                "right_dropped_bundle_count": right.get("dropped_bundle_count"),
            }
        )
    ...
    right_first = first_row(right_path)
    n = max(1, len(pairs))
    summary = {
        "dataset": dataset,
        ...
        "context_token_budget": right_first.get("context_token_budget"),
        "ordering_source": right_first.get("ordering_source"),
        "n": len(pairs),
        ...
        "avg_rendered_bundle_count": right_eval.get("avg_rendered_bundle_count", 0.0),
        "avg_dropped_bundle_count": right_eval.get("avg_dropped_bundle_count", 0.0),
    ...
    stem_parts = [dataset, prompt_profile]
    if right_first.get("top_bundles") is not None:
        stem_parts.append(f"top{right_first.get('top_bundles')}")
    if right_first.get("context_token_budget") is not None:
        stem_parts.append(f"ctx{right_first.get('context_token_budget')}")
    stem_parts.append(str(right_first.get("ordering_source") or "current"))
    stem = "context_budget_compare_" + "_".join(stem_parts)
...
def markdown(summary: Mapping[str, Any]) -> str:
    ...
    lines = [
        ...
        f"- context_token_budget: {summary.get('context_token_budget')}",
        f"- ordering_source: {summary.get('ordering_source')}",
...
def main() -> None:
    ...
    parser.add_argument("--right-truncated", required=True)
    ...
    right_path = Path(args.right_truncated)
